 ...
 * use sample: https://gitlab.dbroscreative.com/project.2017/dev-template-guide-php/blob/master/validator_sample.php
 * git : https://github.com/vlucas/valitron
 */
class Validator
{
    /**
    ...
    public function __construct($data = array(), $fields = array())
		{
  		// injection check
  		$data = array_map('self::requestx', $data);
    ...
    * @return bool
    */
  	public function requestx($value)
  	{
        ...
        $value = str_ireplace('onkeyup','', $value);
  		...
  		$value	= preg_replace_callback($pattern, 'self::patternReplace', $value);
  		return $value;
  	}

  	public function patternReplace($matches)
  	{
  		switch ($matches[0]) {
  			case '"': return '&quot;'; break;
  			case '\'': return '´'; break;
	...
	/**
     * Validate that a field is an Phone (ex. 010-1234-5678 or 01012345678)
     *
     * @param  string $field
     * @param  mixed  $value
     * @return bool
	...
	{
		return preg_match('/^\d{3}-?\d{3,4}-?\d{4}$/i', $value);
	}
    ...
    // Add : Mojito
    public function errorsJsAlert($url = '')
    {
        ...
        $errMsg = '';
        $msgAdd = '';
        foreach( $this->_errors as $paramId=>$paramValue ) {
    		foreach( $paramValue as $errorValue ) {
                $msgAdd .= 'alert(\'' . $errorValue.'\');';
    		}
    	}
        if($url == ''){
            $msgAdd .= 'history.back();';
        }else{
            $msgAdd .= 'location.href="'.$url.'";';
        }


        if (!empty($msgAdd)) $errMsg = "<script>".$msgAdd."</script>";
        echo $errMsg;
    }
